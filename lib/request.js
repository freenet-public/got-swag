...
function request( options ) {
  ...
  if ( h === http && options.agents && options.agents.http ) {
    actual.agent = options.agents.http;
  } else if ( h === https && options.agents && options.agents.https ) {
    actual.agent = options.agents.https;
  } else {
    delete actual.agent;
  }
  ...
  var req = h.request( actual ).on( 'error', function ( err ) {
    ...
    var repost = res.statusCode === 307 || res.statusCode === 308;
    if ( repost && body && typeof body.rewind === 'function' ) {
      body = body.rewind();
    }

    // build follow-up request, passing root
    var follow = request( _.omitBy( {
      method: repost ? method : 'GET',
      url: url.format( location ),
      followRedirects: followRedirects - 1,
      body: repost ? body : undefined,
      root: root,
