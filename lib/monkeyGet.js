...
function monkeyGet( sandbox ) {
  ...
  sandbox.monkeyMemory = listVars( sandbox.vars, sandbox.monkeyMemory );

  var getCandidate = cartesian( parameters.map( function ( parameter ) {
    return [ null ].concat( shuffle( sandbox.monkeyMemory[ parameter.name ] || [] ) );
  } ) );
  ...
  function tryGet( candidate ) {
    ...
    parameters.forEach( function ( parameter, i ) {
      if ( parameter.in === 'path' ) {
        path = path.replace( '{' + parameter.name + '}', candidate[ i ] );
      } else if ( parameter.in === 'query' ) {
        query[ parameter.name ] = candidate[ i ];
      } else if ( parameter.in === 'head' ) {
