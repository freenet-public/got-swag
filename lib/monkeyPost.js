...
function monkeyPost (sandbox){
    ...
    var getCandidate = cartesian( parameters.map( function ( parameter ) {
        if(parameter.in === 'body') {
            ...
            var schemaId = JSON.stringify(parameter.schema);
            if(sandbox.monkeyMemory[schemaId] && sandbox.monkeyMemory[schemaId].length > 0){
                sandbox.monkeyMemory[schemaId].forEach(function(param){
                    temp.push(param);
                })
            }
            return temp;
