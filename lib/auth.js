...
var bufferResponse = require( './bufferResponse' );
...
function auth( options ) {
  ...
  credentials.client_id = credentials.client_id || credentials.api_key || credentials.key;
  credentials.client_secret = credentials.client_secret || credentials.secret;
...
function apiKey( options ) {
  ...
  if ( !credentials.client_id || credentials.client_id.length === 0 ) {
  ...
  case 'query':
    var query = {};
    query[ definition.name ] = client_id;
    return bus.emit( 'auth', { query: query } );

  case 'header':
    var headers = {};
    headers[ definition.name ] = client_id;
...
function oauth2( options ) {
  ...
  bus.on( 'access_token', function ( access_token ) {
    bus.emit( 'auth', bearerToken( access_token ) );
  } );
...
function tryLogin( options ) {
  ...
  req.on( 'final-response', function ( res ) {
    ...
    function nextForm( forms ) {
      ...
      if ( index >= forms.length ) {
        bus.emit( 'error', new Error( 'Could not login (' + res.statusCode + ')' ) );
      }
...
function watchRequest( options ) {
  ...
  req.on( 'redirect', function ( res ) {
    ...
    if ( u && u.hash ) {
      ...
      if ( h.token ) bus.emit( 'access_token', h.token );
      if ( h.access_token ) bus.emit( 'access_token', h.access_token );
      if ( h.code ) bus.emit( 'code', h.code );
    }

  } );

  // on a response, emit access/refresh tokens if present in the json response
  req.on( 'final-response', function ( res ) {

    bufferResponse( res ).on( 'json', function ( json ) {
      if ( json.access_token ) bus.emit( 'access_token', json.access_token );
      if ( json.refresh_token ) bus.emit( 'refresh_token', json.refresh_token );
    } );
